...
def build_market_feature_row(
    ...
    for attr in attrs:
        row[attr] = _serialize(getattr(market_context, attr, None))
...
def build_options_feature_row(
    ...
    row.update(
        {
            "current_iv": _safe_float(getattr(iv_analysis, "current_iv", None), 0.0) if iv_analysis is not None else 0.0,
            "iv_rank": _safe_float(getattr(iv_analysis, "iv_rank", None), 0.0) if iv_analysis is not None else 0.0,
            "iv_percentile": _safe_float(getattr(iv_analysis, "iv_percentile", None), 0.0) if iv_analysis is not None else 0.0,
            "iv_regime": _safe_str(getattr(iv_analysis, "iv_regime", None), "") if iv_analysis is not None else "",
            "iv_hv_ratio": _safe_float(getattr(iv_analysis, "iv_hv_ratio", None), 0.0) if iv_analysis is not None else 0.0,
            "iv_rv_spread": _safe_float(getattr(iv_analysis, "iv_rv_spread", None), 0.0) if iv_analysis is not None else 0.0,
            "skew_legacy": _safe_float(getattr(iv_analysis, "skew", None), 0.0) if iv_analysis is not None else 0.0,
            ...
            "surface_total_contracts": int(_safe_float(surface.total_contracts, 0.0)),
        }
    )
    return row
...
def build_candidate_feature_row(
    ...
    for attr in (
        ...
        row[attr] = _serialize(getattr(candidate, attr, None))
    return row
...
def build_recommendation_feature_row(
    ...
    for attr in (
        ...
        row[attr] = _serialize(getattr(recommendation, attr, None))
    return row
